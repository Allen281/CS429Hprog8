module main_logic(
    ...
    output wire is_write_reg_rslt, is_write_mem_rslt,
    output wire[63:0] read_rslt, rslt_pc, write_data_reg, write_address_mem, write_data_mem
);
    ...
    alu int_arith(opcode, rd, rs, rt, literal, alu_rslt);
    fpu float_arith(opcode, rd, rs, rt, literal, fpu_rslt);
    mov data_movement(clk, opcode, rd, rs, rt, literal, is_write_mem, is_write_reg, read_rslt_mov, write_data_mem, write_address_mem, write_data_reg);
    branch control_flow(clk, opcode, rd, rs, rt, literal, pc, is_write_branch, new_pc, write_address_branch, write_data_branch);
